...
document.addEventListener("click", (ev)=>{
    ...
    if (!rightName) pushNotification(10, 590, "Error", "Employee`s name is not correct\ntoo short", "error");
    if (+a.value < 18 || +a.value > 90 || ageIsNan) pushNotification(10, 300, "Error", "Employee`s age is not correct\nmust be from 18 to 90 and number", "error");
    if (salaryIsNan) pushNotification(10, 880, "Error", "Employee`s salary is not correct\nmust be number", "error");
    if (!rightPosition) pushNotification(10, 1170, "Error", "Employee`s position is not correct\ntoo short", "error");
    if (rightName && +a.value >= 18 && +a.value <= 90 && !ageIsNan && !salaryIsNan && rightPosition) {
        const newTr = document.createElement("tr");
        tableBody.append(newTr);
        newEmployee.forEach((char)=>{
            const td = document.createElement("td");
            if (char.name === "salary") td.innerText = `$${Number(char.value).toLocaleString("en-US")}`;
            else td.innerText = char.value;
            newTr.append(td);
            if (char.name === "office") char.value = char.firstElementChild.value;
            else char.value = "";
        });
        pushNotification(10, 10, "Message", "New employee was added\nSuccessfully", "success");
    }
});
document.addEventListener("click", (ev)=>{
    if (ev.target.parentElement.parentElement.tagName !== "THEAD") return 0;
    ...
    const newList = [
        ...
        if (isNumber) return bValue - aValue;
        else if (isSalary) return makeSalary(bValue) - makeSalary(aValue);
...
document.addEventListener("click", (ev)=>{
    ...
    if (ev.target.parentElement.parentElement.tagName !== "TBODY") return;
    trs.forEach((elem)=>{
...
document.addEventListener("dblclick", (ev)=>{
    ...
    input.focus();
    input.onblur = ()=>{
        if (input.value.trim() === "") td.innerText = tdVal;
